...
class TestFunctionPointerTypes:
    ...
    Ghidra has no anonymous function definition, so applying a signature always creates one.
    An unnamed declarator names it after its own signature; writing a name in the parentheses
    C reserves for one names it instead.
    """
    ...
    def _definitions(self, ghidra_server: GhidraClient, prog: str, query: str) -> list:
        return [t for t in ghidra_server.ok(
            "search_data_types", {"program": prog, "query": query})["data_types"]
            if "FunctionDefinition" in t["type_class"]]
    ...
    def test_declarator_types_a_stack_function_pointer(
        ...
        # The definition is named after the signature, not after the variable: the same
        # spelling elsewhere reaches this same type, and no local name can collide with it.
        assert result["type_name"] == "func_int__int_int *"

        minted = self._definitions(ghidra_server, prog, "func_int__int_int")
        assert [t["name"] for t in minted] == ["func_int__int_int"], minted
        # Behind-the-scenes types stay out of the way of the program's own.
        assert minted[0]["path"] == "/functions", minted
        ...
        assert current["maybe_addFn"] == "func_int__int_int *"
    ...
    def test_declarator_types_a_register_temporary(
        ...
        assert result["type_name"] == "func_void__void *"

    def test_a_name_in_the_declarator_names_the_type(
            self, ghidra_server: GhidraClient, prog: str):
        # C puts a name inside those parentheses; here it names the callback type itself.
        struct = self._struct(ghidra_server, prog, "maybe_NamedCb")
        field = ghidra_server.ok(
            "add_struct_field",
            {"program": prog, "struct_name": struct, "field_name": "maybe_read_0x0",
             "type_name": "int (*maybe_readCb)(int)"},
        )
        assert field["type_name"] == "maybe_readCb *"
        assert [t["name"] for t in self._definitions(ghidra_server, prog, "maybe_readCb")] \
            == ["maybe_readCb"]

    def test_the_same_signature_reuses_the_derived_type(
            self, ghidra_server: GhidraClient, prog: str):
        spelling = "char (*)(int)"
        ...
        for struct in (first, second):
            ...
            assert applied["type_name"] == "func_char__int *"
        # A second definition would land beside the first as func_char__int.conflict.
        assert len(self._definitions(ghidra_server, prog, "func_char__int")) == 1

    def test_different_signatures_never_collide(
            self, ghidra_server: GhidraClient, prog: str):
        first = self._struct(ghidra_server, prog, "maybe_ShapeA")
        second = self._struct(ghidra_server, prog, "maybe_ShapeB")
        # The same field name at two sites, two different signatures: nothing is refused,
        # because neither name is derived from the field.
        one = ghidra_server.ok(
            "add_struct_field",
            {"program": prog, "struct_name": first, "field_name": "maybe_shape_0x0",
             "type_name": "short (*)(int)"},
        )
        two = ghidra_server.ok(
            "add_struct_field",
            {"program": prog, "struct_name": second, "field_name": "maybe_shape_0x0",
             "type_name": "short (*)(int, int)"},
        )
        assert one["type_name"] == "func_short__int *"
        assert two["type_name"] == "func_short__int_int *"

    def test_a_written_name_with_a_different_signature_is_refused(
            self, ghidra_server: GhidraClient, prog: str):
        ...
        ghidra_server.ok(
            ...
            {"program": prog, "struct_name": first, "field_name": "maybe_taken_0x0",
             "type_name": "int (*maybe_takenCb)(int)"},
        )
        ...
        resp = ghidra_server.call(
            "add_struct_field",
            {"program": prog, "struct_name": second, "field_name": "maybe_taken_0x1",
             "type_name": "int (*maybe_takenCb)(int, int)"},
        )
        assert resp["ok"] is False
        error = resp.get("error", "")
        # The name is the one thing in a declarator that does not read as itself, so the
        # message says which name it means, what holds it, and that dropping it is an answer.
        assert "maybe_takenCb" in error
        assert "callback data type" in error
        assert "not the variable, parameter or field being typed" in error
        assert "int (*)(int, int)" in error
        assert "maybe_takenCb *" in error

    def test_a_written_name_taken_by_another_kind_of_type_is_refused(
            self, ghidra_server: GhidraClient, prog: str):
        struct = self._struct(ghidra_server, prog, "maybe_SquattedCb")
        holder = self._struct(ghidra_server, prog, "maybe_Squatter")
        resp = ghidra_server.call(
            "add_struct_field",
            {"program": prog, "struct_name": struct, "field_name": "maybe_squat_0x0",
             "type_name": f"int (*{holder})(int)"},
        )
        assert resp["ok"] is False
        error = resp.get("error", "")
        assert "callback data type" in error and holder in error
        # It is not a function definition, so applying 'name *' is not offered.
        assert f"{holder} *" not in error

    def test_a_declarator_needs_no_name_anywhere(
            self, ghidra_server: GhidraClient, prog: str):
        # Nothing in this call names anything — no new_name, no name in the declarator — and
        # the type is still applied. Retypes the variable the first test in this class named.
        result = ghidra_server.ok(
            "set_variable",
            {"program": prog, "name_or_address": "call_via_ptr",
             "name_or_storage": "maybe_addFn", "type_name": "long (*)(char *)"},
        )
        assert result["type_name"] == "func_long__char_ptr *"

    def test_a_declarator_as_a_return_type(
            self, ghidra_server: GhidraClient, prog: str):
        # A return type has no name of its own, which is no obstacle: the signature names it.
        ghidra_server.ok(
            "set_function_prototype",
            {"program": prog, "name_or_address": "register_churn3",
             "return_type_name": "int (*)(int)"},
        )
        assert "func_int__int *" in ghidra_server.ok(
            "get_function_info",
            {"program": prog, "name_or_address": "register_churn3"})["signature"]

    def test_the_calling_convention_is_part_of_the_derived_name(
            self, ghidra_server: GhidraClient, prog: str):
        conventions = ghidra_server.ok("get_calling_conventions", {"program": prog})
        named = [c for c in conventions["calling_conventions"]
                 if c not in ("unknown", "default", conventions["default_convention"])]
        if not named:
            pytest.skip("this program's compiler spec names no alternative convention")
        convention = named[0]
        struct = self._struct(ghidra_server, prog, "maybe_ConvCb")
        applied = ghidra_server.ok(
            "add_struct_field",
            {"program": prog, "struct_name": struct, "field_name": "maybe_conv_0x0",
             "type_name": f"int ({convention} *)(int)"},
        )
        # Two signatures that differ only in convention are different signatures.
        assert applied["type_name"] != "func_int__int *"
        assert convention.strip("_") in applied["type_name"]
    ...
    def test_nested_function_pointer_parameter_says_to_name_it_first(
        ...
        assert resp["ok"] is False
        error = resp.get("error", "")
        assert "function pointer" in error
        # The way out is the named form, spelled out on the parameter that failed.
        assert "int (*my_callback)(int)" in error
    ...
    def test_declarator_applies_to_a_prototype_and_a_struct_field(
        ...
        ghidra_server.ok(
            ...
            {"program": prog,
             ...
             "parameters": [
                 {"name": "maybe_reader",
                  "type_name": "int (*maybe_readerCb)(void *dst, int nbytes)"},
             ]},
        )
        assert "maybe_readerCb *" in ghidra_server.ok(
            "get_function_info",
